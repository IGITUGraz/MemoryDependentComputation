...
from models.network_models import ReinforcementLearningReLU
from policies.policy import Policy
from storage.rollout_storage import RolloutStorage

parser = argparse.ArgumentParser(description='Reinforcement learning task training with ReLU activations')
...
def main():
    ...
    actor_critic = Policy(
        ...
        net=ReinforcementLearningReLU(input_size=envs.observation_space.shape[0],
    ...
    if log_dir and args.logging and not args.evaluate:
        ...
        log_dir = os.path.join('results', 'runs', time_stamp + '_' + socket.gethostname() +
                               f'_version-{version}-{suffix}_reinforcement-learning-task_relu')
        writer = SummaryWriter(log_dir=log_dir)
...
def save_checkpoint(update: int, actor_critic_state_dict: dict, optimizer_state_dict: dict, args: argparse.Namespace,
    ...
    utils.checkpoint.save_checkpoint({
        ...
        '_' + socket.gethostname() + f'_version-{version}-{suffix}_reinforcement-learning-task_relu'))
