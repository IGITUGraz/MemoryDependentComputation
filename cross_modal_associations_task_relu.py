...
from models.network_models import CrossModalAssociations, CrossModalAssociationsReLU
from models.protonet_models import ProtoNet

parser = argparse.ArgumentParser(description='Cross-modal associations task training with ReLU activations')
...
parser.add_argument('--image_protonet_path', default='',
                    type=str, metavar='PATH', help='Path to the MNIST ProtoNet checkpoint (default: none)')
parser.add_argument('--audio_protonet_path', default='',
...
def main_worker(gpu, num_gpus_per_node, args):
    ...
    if args.use_pretrained_protonet:
        ...
        new_image_protonet_state_dict = OrderedDict()
        for k, v in image_protonet_checkpoint['state_dict'].items():
            if k.startswith('module.'):
                k = k[len('module.'):]  # remove `module.`
            if k.startswith('linear'):
                continue
            else:
                new_image_protonet_state_dict[k] = v

        new_audio_protonet_state_dict = OrderedDict()
        for k, v in audio_protonet_checkpoint['state_dict'].items():
            if k.startswith('module.'):
                k = k[len('module.'):]  # remove `module.`
            if k.startswith('linear'):
                continue
            else:
                new_audio_protonet_state_dict[k] = v

        image_embedding_layer = ProtoNet()
        image_embedding_layer.load_state_dict(new_image_protonet_state_dict)

        mfcc_embedding_layer = ProtoNet()
        image_embedding_layer.load_state_dict(new_audio_protonet_state_dict)
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed and
        ...
        if log_dir and args.logging:
            ...
            log_dir = os.path.join('results', 'runs', time_stamp +
                                   ...
                                   f'_version-{version}-{suffix}_cross-modal-associations_task_relu')
            writer = SummaryWriter(log_dir=log_dir)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed and
            ...
            utils.checkpoint.save_checkpoint({
                ...
                'results', 'checkpoints',
                time_stamp + '_' + socket.gethostname() +
                f'_version-{version}-{suffix}_cross-modal-associations_task_relu'))
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, sample in enumerate(train_loader):
        ...
        # Compute output
        output, encoding_outputs, writing_outputs, reading_outputs = model(
